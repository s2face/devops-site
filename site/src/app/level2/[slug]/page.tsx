...
export async function generateStaticParams() {
  const slugs = await getAllSlugs(2);
  return slugs.map(slug => ({ slug }));
}

export async function generateMetadata({ params }: { params: { slug: string } }): Promise<Metadata> {
  const lesson = await getLessonBySlug(params.slug, 2);
  if (!lesson) return { title: 'Not Found' };

  return {
    title: `${lesson.frontmatter.lesson.title} - Level 2`,
...
export default async function LessonPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="flex mx-auto max-w-7xl">
      <Sidebar currentSlug={lesson.frontmatter.lesson.slug} level={2} />
      <main className="flex-1 p-6 lg:p-10 w-full overflow-hidden">
        <div className="text-sm text-gray-500 dark:text-gray-400 mb-6">
          Главная → Level 2 → Урок {lesson.frontmatter.lesson.number}: {lesson.frontmatter.lesson.title}
        </div>
